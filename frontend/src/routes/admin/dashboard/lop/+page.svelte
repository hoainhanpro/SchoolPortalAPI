...
let faculties = []; // Danh sách khoa để hiển thị dropdown
...
onMount(async () => {
    ...
    if (userDataString) {
        try {
            ...
            // Tải danh sách khoa trước để chuẩn bị cho dropdown
            await fetchFaculties();
            // Sau đó tải danh sách lớp
            await fetchClasses();
        } catch (error) {
...
async function fetchFaculties() {
    try {
        ...
        console.log("Danh sách khoa:", faculties);
    } catch (e) {
...
function openAddForm() {
    ...
    newClass = {
        ...
        ten: '',
        maLop: '',
        khoaId: faculties.length > 0 ? faculties[0].id : null
    };
    showAddForm = true;
}

function openUpdateForm(classItem) {
    currentClass = classItem.lop; 
    newClass = {
        id: classItem.lop.id, 
        ten: classItem.lop.ten, 
        khoaId: classItem.khoa?.id || null 
    };
    showUpdateForm = true;
}
...
async function addClass() {
    try {
        const token = localStorage.getItem('jwtToken');
        const response = await fetch('http://localhost:1234/api/lop', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            if (response.status === 409) { // Check for 409 Conflict status
                throw new Error(errorData.message || "Lớp học với tên và khoa đã tồn tại.");
            } else {
        ...
        successMessage = `Lớp "${newClass.ten}" đã được thêm thành công!`;
...
async function updateClass() {
    try {
        const token = localStorage.getItem('jwtToken');
        const response = await fetch(`http://localhost:1234/api/lop/${currentClass.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
        ...
        updateMessage = `Thông tin lớp "${newClass.ten}" đã được cập nhật!`;
        ...
        alert(`Lỗi khi cập nhật lớp: ${error.message}`);
    }
}
...
async function deleteClass(id) {
    if (confirm("Bạn có chắc chắn muốn xóa lớp này không?")) {
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch(`http://localhost:1234/api/lop/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            ...
            if (response.status === 403) {
                const errorData = await response.json();
                throw new Error(errorData.message || "Bạn không có quyền thực hiện hành động này");
            }
            ...
            deleteMessage = "Lớp đã được xóa thành công!";
...
function getFacultyName(khoaId) {
    const faculty = faculties.find(f => f.id === khoaId);
    return faculty ? faculty.ten : 'Chưa xác định';
}
            ...
            <div class="users-container">
                ...
                {:else if error}
                    <div class="error-container">
                        <p>Lỗi: {error}</p>
                        <button class="secondary-button" on:click={fetchClasses}
                            >Thử lại</button
                        >
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="users-table">
                            ...
                            {#each classes as classItem, index}
                                <tr>
                                    <td>{index+1}</td>
                                    <td>{classItem.lop.ten}</td>
                                    <td>{classItem.khoa ? classItem.khoa.ten : 'Chưa xác định'}</td>
                                    <td class="actions">
                                        <button
                                            class="action-btn edit"
                                            on:click={() => openUpdateForm(classItem)}
                                        >
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                width="16"
                                        ...
                                        <button
                                            ...
                                            on:click={() => deleteClass(classItem.lop.id)}
                                        >
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                width="16"
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={addClass}>
                        <div class="form-group">
                            ...
                            <input type="text" id="ten" bind:value={newClass.ten} required />
                        </div>
                        <div class="form-group">
                            <label for="khoaId">Khoa</label>
                            <select id="khoaId" bind:value={newClass.khoaId} required>
            ...
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={updateClass}>
                        <div class="form-group">
                            <label for="edit-ten">Tên Lớp</label>
                            <input type="text" id="edit-ten" bind:value={newClass.ten} required />
                        </div>
                        <div class="form-group">
                            <label for="edit-khoaId">Khoa</label>
                            <select id="edit-khoaId" bind:value={newClass.khoaId} required>
                                {#each faculties as faculty}
                                    <option value={faculty.id}>{faculty.ten}</option>
                                {/each}
                            </select>
