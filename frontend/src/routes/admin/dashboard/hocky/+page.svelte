<script>
    ...
    let newSemester = {
        ...
        ten: '',
        namHoc: null,
        thuTu: 1,  // Mặc định là học kỳ 1
        startDate: '',
        endDate: ''
    };

    // Theo dõi thay đổi ngày để kiểm tra
    $: if (newSemester.startDate && newSemester.endDate) {
     ...
     function validateDates() {
        ...
        if (endDate <= startDate) {
            dateError = 'Ngày kết thúc phải lớn hơn ngày bắt đầu';
            return false;
        } else {
            dateError = '';
            return true;
    ...
    onMount(async () => {
        ...
        if (userDataString) {
            try {
                ...
                console.log("Authentication successful, fetching semesters");
    ...
    async function fetchSemesters() {
        ...
        try {
            ...
            console.error("Lỗi khi tải dữ liệu học kỳ:", e);
            error = e.message;
            loading = false;
        }
    }
    ...
    function validateForm() {
        ...
        // Kiểm tra tên học kỳ
        if (!newSemester.ten || newSemester.ten.trim() === '') {
            formErrors.ten = 'Tên học kỳ không được để trống';
            isValid = false;
        }

        // Kiểm tra năm học
        if (newSemester.namHoc === null || newSemester.namHoc === '') {
            formErrors.namHoc = 'Năm học không được để trống';
            isValid = false;
        } else if (isNaN(parseInt(newSemester.namHoc))) {
            formErrors.namHoc = 'Năm học phải là số';
            isValid = false;
        }

        // Kiểm tra thứ tự học kỳ
        if (!newSemester.thuTu || ![1, 2, 3].includes(parseInt(newSemester.thuTu))) {
            formErrors.thuTu = 'Thứ tự học kỳ không hợp lệ';
            isValid = false;
        }
        ...
        // Kiểm tra mối quan hệ giữa hai ngày
        if (newSemester.startDate && newSemester.endDate) {
            if (!validateDates()) {
                isValid = false;
            }
    ...
    function openUpdateForm(semester) {
        currentSemester = semester;
        newSemester = {
            id: semester.id,
            ten: semester.name || semester.ten,
            namHoc: semester.namHoc,
            thuTu: semester.thuTu,
            startDate: semester.startDate ? new Date(semester.startDate).toISOString().substring(0, 10) : '',
            endDate: semester.endDate ? new Date(semester.endDate).toISOString().substring(0, 10) : ''
        };
        dateError = '';
        showUpdateForm = true;
    }
    ...
    async function addSemester() {
        ...
        try {
            ...
            const response = await fetch('http://localhost:1234/api/semesters', {
                ...
                body: JSON.stringify({
                    ten: newSemester.ten,
                    namHoc: parseInt(newSemester.namHoc), 
                    thuTu: parseInt(newSemester.thuTu),   
            ...
            if (response.status === 403) {
                ...
                throw new Error(errorData || "Bạn không có quyền thực hiện hành động này");
            }
            
            if (response.status === 409) { // Check for 409 Conflict
                const errorData = await response.json();
                throw new Error(errorData.message || "Năm học, thứ tự học kỳ và tên học kỳ đã tồn tại."); // More specific message
            }
            ...
            successMessage = `Học kỳ "${newSemester.ten}" đã được thêm thành công!`;
            ...
            alert(`Lỗi khi thêm học kỳ: ${error.message}`);
        }
    }
    ...
    async function updateSemester() {
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch(`http://localhost:1234/api/semesters/${currentSemester.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            updateMessage = `Thông tin học kỳ "${newSemester.ten}" đã được cập nhật!`;
    ...
    async function deleteSemester(id) {
        if (confirm("Bạn có chắc chắn muốn xóa học kỳ này không?")) {
            try {
                const token = localStorage.getItem('jwtToken');
                const response = await fetch(`http://localhost:1234/api/semesters/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                ...
                deleteMessage = "Học kỳ đã được xóa thành công!";
    ...
    // Hàm lấy tên thứ tự học kỳ
    function getThuTuLabel(thuTu) {
        const option = thuTuOptions.find(opt => opt.value === thuTu);
...
<div class="app-container">
    ...
    <main class="content">
        <header class="top-bar">
            <h1 class="page-title">Semesters Management</h1>
            ...
            <div class="users-container">
                ...
                {:else if error}
                    <div class="error-container">
                        <p>Lỗi: {error}</p>
                        <button class="secondary-button" on:click={fetchSemesters}
                            >Thử lại</button
                        >
                    </div>
                            ...
                            <tbody>
                                {#each semesters as semester, index}
                                    <tr>
                                        <td>{index+1}</td>
                                        <td>{semester.name}</td>
                                        <td>{semester.namHoc}</td>
                                        <td>
                                            <span class="semester-type semester-type-{semester.thuTu}">
                                                {getThuTuLabel(semester.thuTu)}
                                            </span>
                                        </td>
                                        ...
                                        <td class="actions">
                                            <button
                                                ...
                                                on:click={() => openUpdateForm(semester)}
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    width="16"
        ...
        <div class="modal-overlay">
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={addSemester}>
                        <div class="form-group">
                            <label for="ten">Tên học kỳ</label>
                            <input type="text" id="ten" bind:value={newSemester.ten} required />
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="thuTu">Loại học kỳ</label>
                            <select id="thuTu" bind:value={newSemester.thuTu} required>
                                {#each thuTuOptions as option}
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="startDate" bind:value={newSemester.startDate} required />
                            
                        </div>
                        <div class="form-group">
                            <label for="endDate">Ngày kết thúc</label>
                            <input type="date" id="endDate" bind:value={newSemester.endDate} required />
                            {#if dateError}
                                <div class="error-text">{dateError}</div>
                            {/if}
                        </div>
                        <div class="modal-actions">
                            <button type="submit" class="primary-button" disabled={dateError !== ''}
                                >Thêm Học Kỳ</button
                            >
                            <button
                                type="button"
    ...
    {#if showUpdateForm}
        <div class="modal-overlay">
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={updateSemester}>
                        <div class="form-group">
                            <label for="edit-ten">Tên học kỳ</label>
                            <input type="text" id="edit-ten" bind:value={newSemester.ten} required />
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="edit-thuTu">Loại học kỳ</label>
                            <select id="edit-thuTu" bind:value={newSemester.thuTu} required>
                                {#each thuTuOptions as option}
                                    <option value={option.value}>{option.label}</option>
                                {/each}
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="edit-startDate" bind:value={newSemester.startDate} required />
                        </div>
                        <div class="form-group">
                            <label for="edit-endDate">Ngày kết thúc</label>
                            <input type="date" id="edit-endDate" bind:value={newSemester.endDate} required />
                            {#if dateError}
                                <div class="error-text">{dateError}</div>
                            {/if}
                        </div>
...
<style>
    ...
    /* Additional styles for semesters page */
    .section-header {
        display: flex;
        justify-content: space-between;
        align-items: center;
