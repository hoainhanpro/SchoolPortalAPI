...
onMount(async () => {
    ...
    if (userDataString) {
        try {
            ...
            await Promise.all([fetchUsers(), fetchKhoaList()]);
        } catch (error) {
...
async function fetchKhoaList() {
    const token = localStorage.getItem('jwtToken');
    try {
        const response = await fetch('http://localhost:1234/api/khoa', {
            headers: {
        ...
        if (!response.ok) {
            if (response.status === 401) {
                console.error("Không được phép: 401");
                localStorage.removeItem('jwtToken');
                goto('/');
                return;
            }
            const errorText = await response.text();
            console.error(`Lỗi tải danh sách khoa: Trạng thái ${response.status}, Phản hồi: ${errorText}`);
            throw new Error(`Lỗi tải danh sách khoa: Trạng thái ${response.status}, Phản hồi: ${errorText}`);
        }
        khoaList = await response.json();
    } catch (error) {
        console.error("Lỗi tải danh sách khoa:", error);
        alert(`Có lỗi xảy ra khi tải danh sách khoa: ${error.message}`);
    }
}
...
async function fetchUsers() {
    ...
    try {
        const token = localStorage.getItem('jwtToken');
        const response = await fetch('http://localhost:1234/api/nhanvien', {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
        if (response.ok) {
            ...
            console.error("Lỗi tải người dùng:", response.statusText);
        }
    } catch (err) {
        error = err.message;
    ...
    try {
        const response = await fetch('http://localhost:1234/api/nhanvien', {
            ...
            body: JSON.stringify({
                ...
                email: newUser.email,
                sdt: newUser.sdt,
                gioiTinh: newUser.gioitinh === "1" ? "Nam" : "Nu",
                diaChi: newUser.diachi,
                ngaySinh: newUser.ngaysinh,
                matKhau: newUser.password,
                chucVu: newUser.vaitro === "1" ? "Giao vien" : "Nhan vien",
                khoaId: newUser.khoaId
            })
        });

        if (response.ok) {
            const newUserFromServer = await response.json();
            successMessage = `Người dùng ${newUserFromServer.ho} ${newUserFromServer.ten} đã được thêm thành công!`;
            ...
            console.log("Thêm người dùng thành công:", newUserFromServer);
        } else {
            ...
            } else if (errorMessage.toLowerCase().includes("sdt") || errorMessage.toLowerCase().includes("so dien thoai")) {
                alert("Số điện thoại đã tồn tại!");
            } else {
                alert(`Lỗi khi thêm người dùng: ${errorMessage}`);
            }
        }
    } catch (error) {
...
async function deleteUser(userId) {
    if (confirm("Bạn có chắc chắn muốn xóa nhân viên này không?")) {
        const token = localStorage.getItem('jwtToken');
        try {
            const response = await fetch(`http://localhost:1234/api/nhanvien/${userId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                deleteMessage = data.message || "Nhân viên đã được xóa.";
                setTimeout(() => { deleteMessage = ''; }, 3000);
                fetchUsers();
            } else {
                deleteMessage = "Server trả về phản hồi không phải JSON.";
                setTimeout(() => { deleteMessage = ''; }, 3000);
                console.warn("Phản hồi không phải JSON:", await response.text()); 
            }
        } else {
            deleteMessage = `Lỗi xóa nhân viên: ${response.statusText}`;
            setTimeout(() => { deleteMessage = ''; }, 3000);
        }
        } catch (error) {
            console.error("Lỗi xóa nhân viên:", error);
            deleteMessage = "Lỗi xóa nhân viên: " + error.message;
            setTimeout(() => { deleteMessage = ''; }, 3000); 
        }
 ...
 async function fetchUserById(userId) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Lỗi tải thông tin nhân viên: ${response.status} - ${await response.text()}`);
            }
 ...
 async function updateUser() {
        const token = localStorage.getItem('jwtToken');
        try {
            console.log("Dữ liệu trước khi gửi (updateUser):", newUser);
            ...
            const userData = {
                ...
                diaChi: newUser.diachi,
                ngaySinh: newUser.ngaysinh,
                chucVu: newUser.vaitro === "1" ? "Giang vien" : "Nhan vien",
                khoaId: newUser.khoaId
            };
            
            // Thêm mật khẩu vào dữ liệu gửi đi nếu có nhập mật khẩu mới
            if (newUser.password && newUser.password.trim() !== '') {
                userData.matKhau = newUser.password;
            }

            const response = await fetch(`http://localhost:1234/api/nhanvien/${currentUser.id}`, {
                method: 'PUT',
                headers: {
            ...
            console.log("Trạng thái phản hồi (updateUser):", response.status);
            if (response.ok) {
                updateMessage = `Thông tin người dùng ${newUser.ho} ${newUser.ten} đã được cập nhật!`;
                if (newUser.password && newUser.password.trim() !== '') {
                    updateMessage += ' Mật khẩu đã được thay đổi.';
                }
    ...
    async function openUpdateForm(user) {
        ...
        if (fetchedUser) {
            newUser = {
                ...
                sdt: fetchedUser.sdt || "",
                gioitinh: fetchedUser.gioiTinh === "Nam" ? "1" : "0",
                diachi: fetchedUser.diaChi || "", 
                ngaysinh: fetchedUser.ngaySinh || "",
                password: "", // Reset mật khẩu - để trống
                vaitro: fetchedUser.chucVu === "Giao vien" ? "1" : "0",
                khoaId: fetchedUser.khoaId || null
            };
        } else {
            ...
            <div class="users-container">
                ...
                {:else if error}
                    <div class="error-container">
                        <p>Error: {error}</p>
                        <button class="secondary-button" on:click={fetchUsers}
                            >Try Again</button
                        >
                    </div>
                            ...
                            <tbody>
                                {#each users as user, index}
                                    <tr>
                                        ...
                                        <td>{user.ngaySinh}</td>
                                        <td>{user.gioiTinh === null ? "-" : (user.gioiTinh === "Nam" ? "Nam" : "Nữ")}</td>
                                        <td>{user.sdt || "-"}</td>
                                        <td>{user.email}</td>
                                        <td>{user.chucVu === "Giao vien" ? "Giáo viên" : (user.chucVu === "Nhan vien" ? "Nhân viên" : user.chucVu)}</td>
                                        <td>{user.khoaTen || '-'}</td>
                                        <td class="actions">
                                            <button
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={addUser}>
                        <div class="form-group">
                            ...
                            <input type="text" id="ho" bind:value={newUser.ho} required />
                        </div>
                        <div class="form-group">
                            <label for="ten">Tên</label>
                            <input type="text" id="ten" bind:value={newUser.ten} required />
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input
                                    type="date"
                                    id="ngaysinh"
                                    bind:value={newUser.ngaysinh}
                                />
                            </div>

                            <div class="form-group">
                                <label for="vaitro">Chức vụ</label>
                                <select id="vaitro" bind:value={newUser.vaitro}>
                        ...
                        <div class="form-group">
                            ...
                            <select id="khoaId" bind:value={newUser.khoaId}>
                                {#each khoaList as khoa}
    ...
    {#if showUpdateForm}
        <div class="modal-overlay">
            <div class="modal">
                <div class="modal-header">
                    <h3>Update User</h3>
                    <button class="close-button" on:click={() => (showUpdateForm = false)}>×</button>
                </div>
                <div class="modal-body">
                    <form on:submit|preventDefault={updateUser}>
                        ...
                        <div class="modal-actions">
                            ...
                            <button type="button" class="secondary-button" on:click={() => (showUpdateForm = false)}>Cancel</button>
                        </div>
                    </form>
                </div>
            </div>
