...
onMount(async () => {
    ...
    if (userDataString) {
        try {
            ...
            console.log("Authentication successful, fetching faculties");
...
async function fetchFaculties() {
        ...
        try {
            ...
            console.error("Lỗi khi tải dữ liệu khoa:", e);
            error = e.message;
            loading = false;
        }
    }
    ...
    function openUpdateForm(faculty) {
    ...
    async function addFaculty() {
        try {
            ...
            const response = await fetch('http://localhost:1234/api/khoa', {
                ...
                body: JSON.stringify({ ten: newFaculty.ten })
            });
            ...
            if (!response.ok) {
                if (response.status === 409) {
                    const errorData = await response.json();
                    alert(errorData.message || "Tên khoa đã tồn tại!"); 
                } else {
                    const errorData = await response.json();
            ...
            successMessage = `Khoa "${newFaculty.ten}" đã được thêm thành công!`;
            ...
            alert(`Lỗi khi thêm khoa: ${error.message}`);
        }
    }
    ...
    async function updateFaculty() {
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch(`http://localhost:1234/api/khoa/${currentFaculty.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                },
                body: JSON.stringify({ id: currentFaculty.id, ten: newFaculty.ten })
            });
            ...
            updateMessage = `Thông tin khoa "${newFaculty.ten}" đã được cập nhật!`;
            ...
            alert(`Lỗi khi cập nhật khoa: ${error.message}`);
        }
    }
    ...
    async function deleteFaculty(id) {
        if (confirm("Bạn có chắc chắn muốn xóa khoa này không?")) {
            try {
                const token = localStorage.getItem('jwtToken');
                const response = await fetch(`http://localhost:1234/api/khoa/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                ...
                deleteMessage = "Khoa đã được xóa thành công!";
                setTimeout(() => { deleteMessage = ''; }, 3000);
                
                await fetchFaculties();
            } catch (error) {
                console.error("Lỗi khi xóa khoa:", error);
                deleteMessage = `Lỗi: ${error.message}`;
            ...
            <div class="users-container">
                ...
                {:else if error}
                    <div class="error-container">
                        <p>Lỗi: {error}</p>
                        <button class="secondary-button" on:click={fetchFaculties}
                            >Thử lại</button
                        >
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="actions">
                                            <button
                                                ...
                                                on:click={() => openUpdateForm(faculty)}
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    width="16"
        ...
        <div class="modal-overlay">
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={addFaculty}>
                        <div class="form-group">
                            <label for="ten">Tên khoa</label>
                            <input type="text" id="ten" bind:value={newFaculty.ten} required />
                        </div>
    ...
    {#if showUpdateForm}
        <div class="modal-overlay">
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={updateFaculty}>
                        <div class="form-group">
                            <label for="edit-ten">Tên khoa</label>
                            <input type="text" id="edit-ten" bind:value={newFaculty.ten} required />
                        </div>
                        <div class="modal-actions">
                            <button type="submit" class="primary-button"
                                >Lưu Thay Đổi</button
...
<style>
    ...
    .modal-overlay {
        ...
        right: 0;
        bottom: 0;
        background-color: rgba(175, 32, 32, 0.5);
        display: flex;
        align-items: center;
        justify-content: center;
        z-index: 1000;
