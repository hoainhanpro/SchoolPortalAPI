<script>
    ...
    function validateDateTime() {
        if (!newDotDangKy.thoiGianBatDau || !newDotDangKy.thoiGianKetThuc) {
            return true; 
        }

        const startDate = new Date(newDotDangKy.thoiGianBatDau);
        const endDate = new Date(newDotDangKy.thoiGianKetThuc);
        
        if (endDate <= startDate) {
            validationError = "Thời gian kết thúc phải lớn hơn thời gian bắt đầu";
            return false;
        }
    ...
    async function fetchDotDangKys() {
        loading = true;
        error = null;
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch('http://localhost:1234/api/dotdangky', {
            ...
            dotDangKys = await response.json();
        } catch (err) {
            console.error("Error fetching DotDangKys:", err);
            error = err.message;
        } finally {
            loading = false;
        }
    ...
    async function addDotDangKy() {
        ...
        try {
            ...
            const response = await fetch('http://localhost:1234/api/dotdangky', {
                ...
                body: JSON.stringify(newDotDangKy)
            });
            
            if (response.status === 409) {
                // Xử lý lỗi 409 - Conflict (học kỳ đã tồn tại đợt đăng ký)
                const errorData = await response.json();
                errorMessage = errorData.message || "Đã tồn tại đợt đăng ký cho học kỳ này!";
            ...
            successMessage = "Đợt đăng ký đã được thêm thành công!";
            setTimeout(() => successMessage = "", 3000);
            showAddForm = false;
            clearForm();
            await fetchDotDangKys();
        } catch (error) {
            console.error("Error adding DotDangKy:", error);
            errorMessage = `Lỗi khi thêm đợt đăng ký: ${error.message}`;
            setTimeout(() => errorMessage = "", 5000);
        }
    }

    async function updateDotDangKy(id) {
        // Kiểm tra thời gian trước khi gọi API
        if (!validateDateTime()) {
            return;
        }
        ...
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch(`http://localhost:1234/api/dotdangky/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            if (response.status === 409) {
                const errorData = await response.json();
                errorMessage = errorData.message || "Đã tồn tại đợt đăng ký khác cho học kỳ này!";
                setTimeout(() => errorMessage = "", 5000);
                return;
            }
            ...
            updateMessage = "Đợt đăng ký đã được cập nhật thành công!";
            setTimeout(() => updateMessage = "", 3000);
            showUpdateForm = false;
            await fetchDotDangKys();
        } catch (error) {
            console.error("Error updating DotDangKy:", error);
            errorMessage = `Lỗi khi cập nhật đợt đăng ký: ${error.message}`;
            setTimeout(() => errorMessage = "", 5000);
        }
    }
    ...
    function prepareUpdateForm(dotDangKy) {
        newDotDangKy = {
            id: dotDangKy.id,
            hocKyId: dotDangKy.hocKyId,
            thoiGianBatDau: formatDateForInput(dotDangKy.thoiGianBatDau),
            thoiGianKetThuc: formatDateForInput(dotDangKy.thoiGianKetThuc),
        };
        validationError = "";
        showUpdateForm = true;
    }

    // Hàm định dạng ngày cho input datetime-local
    function formatDateForInput(dateString) {
        ...
        // Định dạng yyyy-MM-ddThh:mm:ss cho input datetime-local
...
<div class="app-container">
    ...
    <main class="content">
        <header class="top-bar">
            <h1 class="page-title">Quản Lý Đợt Đăng Ký Lớp Tín Chỉ</h1>
        ...
        <div class="page-content">
            <div class="users-container">
                ...
                <div class="section-header">
                    <h2>Danh Sách Đợt Đăng Ký Lớp Tín Chỉ</h2>
                ...
                {:else if error}
                    <div class="error-container">
                        <p>Lỗi: {error}</p>
                        <button class="secondary-button" on:click={fetchDotDangKys}>Thử lại</button>
                    </div>
                {:else if dotDangKys.length === 0}
                    <div class="empty-container">
                        <p>Không tìm thấy đợt đăng ký nào.</p>
                    </div>
                {:else}
                    <div class="table-responsive">
                        <table class="users-table">
                            ...
                            <tbody>
                                {#each dotDangKys as dotDangKy, index}
                                    <tr>
                                        <td>{index+1}</td>
                                        <td>{findHocKyName(dotDangKy.hocKyId)}</td>
                                        <td>{formatDateTime(dotDangKy.thoiGianBatDau)}</td>
                                        <td>{formatDateTime(dotDangKy.thoiGianKetThuc)}</td>
                                        <td class="actions">
                                            <button class="action-btn edit" on:click={() => prepareUpdateForm(dotDangKy)}>
        ...
        <div class="modal-overlay">
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={addDotDangKy}>
                        <div class="form-group">
                            <label for="add-hocKyId">Học kỳ</label>
                            <select id="add-hocKyId" bind:value={newDotDangKy.hocKyId} required>
                        ...
                        <div class="form-group">
                            <label for="add-thoiGianBatDau">Thời gian bắt đầu</label>
                            <input 
                                type="datetime-local" 
                                id="add-thoiGianBatDau" 
                                bind:value={newDotDangKy.thoiGianBatDau} 
                                step="1" 
                                required 
                            />
                            <small class="form-text text-muted">Định dạng: DD/MM/YYYY HH:MM:SS (24 giờ)</small>
                        </div>
                        <div class="form-group">
                            <label for="add-thoiGianKetThuc">Thời gian kết thúc</label>
                            <input 
                                type="datetime-local" 
                                id="add-thoiGianKetThuc" 
                                bind:value={newDotDangKy.thoiGianKetThuc} 
                                step="1" 
                                required 
                            />
                            <small class="form-text text-muted">Định dạng: DD/MM/YYYY HH:MM:SS (24 giờ)</small>
                        ...
                        <div class="modal-actions">
                            <button type="submit" class="primary-button" disabled={!!validationError}>Thêm Mới</button>
                            <button
                                type="button"
    ...
    {/if}
     ...
     <!-- Edit DotDangKy Modal -->
    {#if showUpdateForm}
        <div class="modal-overlay">
            <div class="modal">
                <div class="modal-header">
                    <h3>Chỉnh Sửa Đợt Đăng Ký</h3>
                    <button
                        class="close-button"
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={() => updateDotDangKy(newDotDangKy.id)}>
                        <div class="form-group">
                            <label for="edit-hocKyId">Học kỳ</label>
                            <select id="edit-hocKyId" bind:value={newDotDangKy.hocKyId} required>
                                <option value="">-- Chọn học kỳ --</option>
                                {#each hocKys as hocKy}
                                    <option value={hocKy.id}>{hocKy.ten} {hocKy.namHoc} ({hocKy.thuTu})</option>
                                {/each}
                        ...
                        <div class="form-group">
                            <label for="edit-thoiGianBatDau">Thời gian bắt đầu</label>
                            <input type="datetime-local" id="edit-thoiGianBatDau" bind:value={newDotDangKy.thoiGianBatDau} step="1" required />
                            <small class="form-text text-muted">Định dạng: DD/MM/YYYY HH:MM:SS (24 giờ)</small>
                        </div>
                        <div class="form-group">
                            <label for="edit-thoiGianKetThuc">Thời gian kết thúc</label>
                            <input type="datetime-local" id="edit-thoiGianKetThuc" bind:value={newDotDangKy.thoiGianKetThuc} step="1" required />
                            <small class="form-text text-muted">Định dạng: DD/MM/YYYY HH:MM:SS (24 giờ)</small>
                        </div>
...
<style>
    /* Các styles giữ nguyên từ file mẫu */
    /* Base layout styles */
    :global(body) {
        margin: 0;
