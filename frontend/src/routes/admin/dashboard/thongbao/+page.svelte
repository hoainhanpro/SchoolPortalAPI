<script>
    ...
    function validateThongBao() {
        if (!newThongBao.tieuDe || newThongBao.tieuDe.trim() === '') {
            validationError = "Tiêu đề thông báo không được để trống";
            return false;
        }
        
        if (!newThongBao.noiDung || newThongBao.noiDung.trim() === '') {
            validationError = "Nội dung thông báo không được để trống";
            return false;
        }
    ...
    async function fetchThongBaos() {
        loading = true;
        error = null;
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch('http://localhost:1234/api/thongbao', {
            ...
            thongBaos = await response.json();
        } catch (err) {
            console.error("Error fetching ThongBaos:", err);
            error = err.message;
        } finally {
            loading = false;
        }
    ...
    async function addThongBao() {
        ...
        try {
            ...
            const response = await fetch('http://localhost:1234/api/thongbao', {
                ...
                body: JSON.stringify(newThongBao)
            });
            ...
            successMessage = "Thông báo đã được tạo thành công!";
            setTimeout(() => successMessage = "", 3000);
            showAddForm = false;
            clearForm();
            await fetchThongBaos();
        } catch (error) {
            console.error("Error adding ThongBao:", error);
            errorMessage = `Lỗi khi thêm thông báo: ${error.message}`;
    ...
    async function updateThongBao(id) {
        ...
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch(`http://localhost:1234/api/thongbao/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            updateMessage = "Thông báo đã được cập nhật thành công!";
            setTimeout(() => updateMessage = "", 3000);
            showUpdateForm = false;
            await fetchThongBaos();
        } catch (error) {
            console.error("Error updating ThongBao:", error);
            errorMessage = `Lỗi khi cập nhật thông báo: ${error.message}`;
            setTimeout(() => errorMessage = "", 5000);
        }
    }
    ...
    async function deleteThongBao(id) {
        ...
        try {
            ...
            successMessage = "Thông báo đã được xóa thành công!";
            setTimeout(() => successMessage = "", 3000);
            await fetchThongBaos();
        } catch (error) {
            console.error("Error deleting ThongBao:", error);
            errorMessage = `Lỗi khi xóa thông báo: ${error.message}`;
            setTimeout(() => errorMessage = "", 5000);
        }
    }
    ...
    function prepareUpdateForm(thongBao) {
        newThongBao = {
            id: thongBao.id,
            nvId: thongBao.nvId,
    ...
    function viewThongBao(thongBao) {
        newThongBao = {
            id: thongBao.id,
            nvId: thongBao.nvId,
            tieuDe: thongBao.tieuDe,
    ...
    async function fetchUserInfo(token) {
        try {
            ...
            if (response.ok) {
                ...
                // Đặt ID nhân viên hiện tại vào newThongBao
                newThongBao.nvId = userData.id;
            ...
            <div class="users-container">
                ...
                {:else if error}
                    <div class="error-container">
                        <p>Lỗi: {error}</p>
                        <button class="secondary-button" on:click={fetchThongBaos}>Thử lại</button>
                    </div>
                {:else if thongBaos.length === 0}
                ...
                {:else}
                    <div class="table-responsive">
                        <table class="users-table">
                            ...
                            <tbody>
                                {#each thongBaos as thongBao, index}
                                    <tr>
                                        <td>{index+1}</td>
                                        <td>{thongBao.hoNhanVien} {thongBao.tenNhanVien}</td>
                                        <td>{thongBao.tieuDe}</td>
                                        <td>{truncateText(thongBao.noiDung)}</td>
                                        <td>{formatDateTime(thongBao.ngayGui)}</td>
                                        <td class="actions">
                                            <button class="action-btn view" on:click={() => viewThongBao(thongBao)} title="Xem chi tiết">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                                    <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8zM1.173 8a13.133 13.133 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13.133 13.133 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5c-2.12 0-3.879-1.168-5.168-2.457A13.134 13.134 0 0 1 1.172 8z"/>
                                                    <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zM4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0z"/>
                                                </svg>
                                            </button>
                                            <button class="action-btn edit" on:click={() => prepareUpdateForm(thongBao)} title="Chỉnh sửa">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
        ...
        <div class="modal-overlay">
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={addThongBao}>
                        <div class="form-group">
                            <label for="tieuDe">Tiêu đề</label>
                            <input type="text" id="tieuDe" bind:value={newThongBao.tieuDe} required />
                        </div>
                        <div class="form-group">
                            <label for="noiDung">Nội dung</label>
                            <textarea id="noiDung" bind:value={newThongBao.noiDung} rows="10" required></textarea>
                        </div>
                        ...
                        <div class="modal-actions">
                            <button type="submit" class="primary-button" disabled={!!validationError}>Tạo Thông Báo</button>
                            <button
                                type="button"
    ...
    {#if showUpdateForm}
        <div class="modal-overlay">
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={() => updateThongBao(newThongBao.id)}>
                        <div class="form-group">
                            <label for="edit-tieuDe">Tiêu đề</label>
                            <input type="text" id="edit-tieuDe" bind:value={newThongBao.tieuDe} required />
                        </div>
                        <div class="form-group">
                            <label for="edit-noiDung">Nội dung</label>
                            <textarea id="edit-noiDung" bind:value={newThongBao.noiDung} rows="10" required></textarea>
                        </div>
        ...
        <div class="modal-overlay">
            <div class="modal">
                <div class="modal-header">
                    ...
                    <button
                        class="close-button"
                        on:click={() => (showViewForm = false)}>×</button
                    >
                </div>
                <div class="modal-body">
                    <div class="view-info">
                        <h4 class="view-title">{newThongBao.tieuDe}</h4>
                        <div class="view-content">
                            <p>{newThongBao.noiDung}</p>
                        </div>
                    </div>
                    <div class="modal-actions">
                        <button
                            type="button"
                            class="secondary-button"
                            on:click={() => (showViewForm = false)}>Đóng</button
                        >
                    </div>
                </div>
            </div>
...
<style>
    /* Giữ nguyên các styles từ file mẫu và bổ sung thêm styles mới */
    /* Base layout styles */
    :global(body) {
        margin: 0;
