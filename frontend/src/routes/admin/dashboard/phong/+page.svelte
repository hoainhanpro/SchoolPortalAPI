<script>
    ...
    // Phòng học mới hoặc cần cập nhật
    let newRoom = {
        id: null,
        ten: ""
    };
    ...
    async function addRoom(event) {
        try {
            ...
            if (response.status === 409) {
                ...
                throw new Error(errorText || "Phòng học đã tồn tại");
            }
            ...
            successMessage = `Phòng học "${newRoom.ten}" đã được thêm thành công!`;
            ...
            alert(`Lỗi khi thêm phòng học: ${error.message}`);
        }
    }
    ...
    async function updateRoom() {
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch(`http://localhost:1234/api/phong-hoc/${newRoom.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            updateMessage = `Thông tin phòng học "${newRoom.ten}" đã được cập nhật!`;
            ...
            alert(`Lỗi khi cập nhật phòng học: ${error.message}`);
        }
    }
    ...
    async function deleteRoom(id) {
        if (confirm("Bạn có chắc chắn muốn xóa phòng học này không?")) {
            try {
                const token = localStorage.getItem('jwtToken');
                const response = await fetch(`http://localhost:1234/api/phong-hoc/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                ...
                if (response.status === 409) {
                    const errorText = await response.text();
                    throw new Error(errorText || "Không thể xóa phòng học này vì đang được sử dụng");
                }
                ...
                deleteMessage = "Phòng học đã được xóa thành công!";
            ...
            <div class="users-container">
                ...
                {:else if error}
                    <div class="error-container">
                        <p>Lỗi: {error}</p>
                        <button class="secondary-button" on:click={fetchRooms}
                            >Thử lại</button
                        >
                    </div>
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={addRoom}>
                        <div class="form-group">
                            ...
                            <input type="text" id="ten" bind:value={newRoom.ten} required />
                        </div>
            ...
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={updateRoom}>
                        <div class="form-group">
                            <label for="edit-ten">Tên phòng học</label>
                            <input type="text" id="edit-ten" bind:value={newRoom.ten} required />
                        </div>
                        <div class="modal-actions">
                            <button type="submit" class="primary-button"
                                >Lưu Thay Đổi</button
