<script>
    ...
    async function addSubject(event) {
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch('http://localhost:1234/api/mon', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    soTinChi: Number(newSubject.soTinChi),
                    khoa_id: Number(newSubject.khoa_id) || null
                })
            });
            ...
            if (response.status === 409) { // Check for 409 Conflict
                const errorData = await response.json();
                throw new Error(errorData.message || "Môn học đã tồn tại trong khoa."); // More specific message
            }
            ...
            successMessage = `Môn học "${newSubject.tenMon}" đã được thêm thành công!`;
    ...
    async function updateSubject() {
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await fetch(`http://localhost:1234/api/mon/${newSubject.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            updateMessage = `Thông tin môn học "${newSubject.tenMon}" đã được cập nhật!`;
            ...
            alert(`Lỗi khi cập nhật môn học: ${error.message}`);
        }
    }
    ...
    async function deleteSubject(id) {
        if (confirm("Bạn có chắc chắn muốn xóa môn học này không?")) {
            try {
                const token = localStorage.getItem('jwtToken');
                const response = await fetch(`http://localhost:1234/api/mon/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                ...
                if (response.status === 403) {
                    ...
                    throw new Error(errorText || "Bạn không có quyền thực hiện hành động này");
                }
                
                if (response.status === 409) {
                    const errorText = await response.text();
                    throw new Error(errorText || "Không thể xóa môn học này vì đang được sử dụng");
                }
                ...
                deleteMessage = "Môn học đã được xóa thành công!";
    ...
    // Thêm hàm fetchUserInfo từ đoạn code 1
    async function fetchUserInfo(token) {
        try {
            ...
            if (response.ok) {
                ...
                // Kiểm tra quyền truy cập - có thể điều chỉnh theo yêu cầu của bạn
                ...
                await Promise.all([fetchSubjects(), fetchFaculties()]);
            } else {
            ...
            <div class="users-container">
                ...
                {:else if error}
                    <div class="error-container">
                        <p>Lỗi: {error}</p>
                        <button class="secondary-button" on:click={fetchSubjects}
                            >Thử lại</button
                        >
                    </div>
                            ...
                            <tbody>
                                {#each subjects as subject, index}
                                    <tr>
                                        ...
                                        <td>{subject.ten_khoa || 'Không có'}</td>
                                        <td class="actions">
                                            <button
                                                class="action-btn edit"
                                                on:click={() => openUpdateForm(subject)}
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    width="16"
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={addSubject}>
                        <div class="form-group">
                            ...
                            <input type="text" id="tenMon" bind:value={newSubject.tenMon} required />
                        </div>
                        <div class="form-group">
                            <label for="soTinChi">Số tín chỉ</label>
                            <input type="number" id="soTinChi" bind:value={newSubject.soTinChi} min="1" max="10" required />
                        </div>
                        <div class="form-group">
                            <label for="khoa_id">Khoa</label>
                            <select id="khoa_id" bind:value={newSubject.khoa_id}>
                                <option value={null}>-- Chọn khoa --</option>
                                {#each faculties as faculty}
                                    <option value={faculty.id}>{faculty.ten}</option>
                                {/each}
                            </select>
                        </div>
                        <div class="modal-actions">
                            <button type="submit" class="primary-button">Thêm Môn Học</button>
                            <button
                                type="button"
            ...
            <div class="modal">
                ...
                <div class="modal-body">
                    <form on:submit|preventDefault={updateSubject}>
                        <div class="form-group">
                            <label for="edit-tenMon">Tên môn học</label>
                            <input type="text" id="edit-tenMon" bind:value={newSubject.tenMon} required />
                        </div>
                        <div class="form-group">
                            <label for="edit-soTinChi">Số tín chỉ</label>
                            <input type="number" id="edit-soTinChi" bind:value={newSubject.soTinChi} min="1" max="10" required />
                        </div>
                        <div class="form-group">
                            <label for="edit-khoa_id">Khoa</label>
                            <select id="edit-khoa_id" bind:value={newSubject.khoa_id}>
                                <option value={null}>-- Chọn khoa --</option>
                                {#each faculties as faculty}
                                    <option value={faculty.id}>{faculty.ten}</option>
                                {/each}
